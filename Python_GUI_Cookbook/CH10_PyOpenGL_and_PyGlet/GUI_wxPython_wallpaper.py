'''
Using a bitmap as a background. Tiled the
bitmap image and, when we resized the GUI window, the bitmap automatically adjusted
itself to fill in the entire area of the canvas painting on using the device context.
'''
...
class GUI(wx.Panel):
    def __init__(self, parent):
        ...
        parent.Bind(wx.EVT_SIZE, self.canvasCallback)         # react to a resize event and redraw image
    ...
    def canvasCallback(self, event=None):
        dc = wx.ClientDC(self)         # create the device context
