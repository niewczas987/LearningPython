...
class OOP():
    ...
    def click(self):
        ...
        self.a_label.configure(text=self.i18n.colors[2])
    ...
    def radCall(self):
        ...
        if self.redSel == 0: self.mighty2.configure(text=self.i18n.WIDGET_LABEL + self.i18n.colorsIn[0])
        elif self.redSel == 1: self.mighty2.configure(text=self.i18n.WIDGET_LABEL + self.i18n.colorsIn[1])
        elif self.redSel == 2: self.mighty2.configure(text=self.i18n.WIDGET_LABEL + self.i18n.colorsIn[2])
    ...
    def insert_quote(self):
        title = self.book_name.get()
        page = self.page_nr.get()
        quote = self.quot_scr.get(1.0,tk.END)
        #print(title)
        #print(quote)
        self.sql_obj.insertBooks(title,page,quote)

    def get_books(self):
        self.quot_scr.delete(1.0,tk.END)
        all_books = self.sql_obj.showBooks()
        for book in all_books:
            self.quot_scr.insert(tk.INSERT, book[1]+'\n')

    def get_quotes(self):
        self.quot_scr.delete(1.0,tk.END)
        all_books = self.sql_obj.showQuotes()
        for book in all_books:
            self.quot_scr.insert(tk.INSERT, 'Book: %s\n Quote: %s'%(book[1], book[0])+'\n')
    ...
    def create_widgets(self):
        ...
        self.mighty = ttk.LabelFrame(self.tab1, text=self.i18n.labelsFrame)
        self.mighty.grid(column=0, row=0, padx=8, pady=4)
        self.mighty2 = ttk.LabelFrame(self.tab2, text=self.i18n.label2)
        ...
        #add buttons
        self.insert_quote_btn = ttk.Button(self.sql_cont,width=15, text='Insert quote', command=self.insert_quote)
        self.insert_quote_btn.grid(column=3, row=0)
        self.get_quotes_btn = ttk.Button(self.sql_cont,width=15, text='Get books', command=self.get_books)
        self.get_quotes_btn.grid(column=3, row=1)
        self.modify_quote_btn = ttk.Button(self.sql_cont,width=15, text='Get quotes', command=self.get_quotes)
        ...
        self.check1 = tk.Checkbutton(self.mighty2, text=self.i18n.disabled, variable=self.chVarDis, state='disabled')
        self.check1.select()
        self.check1.grid(column=0, row=3)

        self.chVarUn = tk.IntVar()
        self.check2 = tk.Checkbutton(self.mighty2, text=self.i18n.unchecked, variable=self.chVarUn)
        self.check2.deselect()
        self.check2.grid(column=1, row=3)

        self.chVarEn = tk.IntVar()
        self.check3 = tk.Checkbutton(self.mighty2, text=self.i18n.toggle, variable=self.chVarEn)
        self.check3.select()
        ...
        for col in range(3):
            self.curRad = tk.Radiobutton(self.mighty2, text=self.i18n.colors[col], variable=self.radVar, value=col, command=self.radCall)
        ...
        #adding button frame
        self.buttons_frame = ttk.LabelFrame(self.mighty2, text=self.i18n.labelsFrame)
        ...
        mngFilesFrame = ttk.LabelFrame(self.tab2, text=self.i18n.manageFiles)
        mngFilesFrame.grid(column=0, row=4, sticky='WE', padx=10, pady=5)
        # add button to browse files
        lb = ttk.Button(mngFilesFrame, text=self.i18n.browseTo, command=self.get_file_name)
        ...
        cb = ttk.Button(mngFilesFrame, text=self.i18n.copyFileTo, command=self.copy_file)
